...
The outer `.app-shell` centers one `.sheet-frame` and uses a `clamp(18px, 3.5vw, 48px)` browser-edge padding. Sheets are capped at `1480px`, carry a 1px border, and keep a 14px inset registration rule. The landing surface uses a two-column grid (`0.77fr / 1.23fr`) with a left promise and right sample tracer. The inspector uses three flexible columns for Structure tree, dominant Bytes, and Field inspector, with the middle panel receiving the widest measure.
