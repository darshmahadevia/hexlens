...
function startFileJob(file: File, origin: View): void {
  ...
  const jobId = fileJobs.start(file, {
    ...
    onAccepted: (inspection, acceptedFile, callbackJobId) => {
      ...
      render();
      publishImmediateAnnouncement(`Opened ${acceptedFile.type === 'image/png' ? 'a local PNG' : 'a local file as PNG'}. File data remains in memory only.`);
...
function renderDiagnostics(inspection: Inspection): string {
  ...
  const items = inspection.diagnostics.map((diagnostic) => `<li><code>${escapeHtml(diagnostic.code)}</code><span><strong class="diagnostic-severity">${escapeHtml(diagnostic.severity)}</strong> ${escapeHtml(diagnostic.message)}</span><small>${spanLabel(diagnostic.span)}</small></li>`).join('');
